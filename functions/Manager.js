...
exports.manage = async function (client, message, cmd) {
    ...
    if (cmdData.timeoutReset > now && cmdData.UsedCommandsWarn !== 25 && cmdData.GlobalWarns !== 3) {
        cmdData.UsedCommandsWarn += Math.floor(1);
        cmdData.UsedCommandsWarnsReset = Math.floor(Date.now() + UsedCommandsWarnsResetduration);
        await cmdData.save()
      } else if(cmdData.UsedCommandsWarnsReset < now) {
        cmdData.UsedCommandsWarn = Math.floor(0);
        await cmdData.save()
      } else if(cmdData.UsedCommandsWarn === 25 && cmdData.GlobalWarns !== 3) {
        cmdData.GlobalWarns += Math.floor(1);
        cmdData.UsedCommandsWarn = Math.floor(0);
        cmdData.GlobalWarnsReset = Math.floor(Date.now() + GlobalWarnsResetResetduration);
        await cmdData.save()
      } else if(cmdData.GlobalWarnsReset < now) {
        cmdData.GlobalWarns = Math.floor(0);
        await cmdData.save()
