...
exports.Level = async function (message) {
  ...
  if (Userdata.System.xp >= Userdata.System.required) {
    Userdata.System.required = Math.floor(
      (Userdata.System.level + 1) * (Userdata.System.level + 1) * 100
    );
    ...
    message.channel.send({ embeds: [LevelUp] }).then((msg) => {
      ...
      let role = data.Mod.Level.Roles.filter(
        (x) => x.Level == Userdata.System.level
      );
      if (role) {
