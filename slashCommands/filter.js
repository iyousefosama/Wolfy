...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                ...
                .addStringOption(option => option.setName('input').setDescription('Enter an input').setRequired(true)))
        .addSubcommand(subcommand =>
            subcommand
                .setName('captcha')
    ...
    async execute(client, interaction) {
        ...
        if (interaction.options.getSubcommand() === 'blurpify') {
            ...
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "blurpify.png" });
            await interaction.editReply({files: [attachment]});
        } else if (interaction.options.getSubcommand() === 'magik') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=magik&image=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "magik.png" });
            await interaction.editReply({files: [attachment]});
        } else if (interaction.options.getSubcommand() === 'deepfry') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=deepfry&image=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "deepfry.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'awooify') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=awooify&url=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "awooify.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'baguette') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=baguette&url=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "baguette.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'trash') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=trash&url=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "trash.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'jpeg') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=jpeg&url=${interaction.user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "jpeg.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'lolice') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=lolice&url=${interaction.user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message, "lolice.png");
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'trap') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=trap&name=${user.username}&author=${interaction.user.username}&image=${user.displayAvatarURL({dynamic: true, extension:'png'})}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "trap.png" });
            await interaction.editReply({files: [attachment]});
            ...
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "changemymind.png" });
            await interaction.editReply({files: [attachment]});
        } else if(interaction.options.getSubcommand() === 'captcha') {
            let res = await fetch(encodeURI(`https://nekobot.xyz/api/imagegen?type=captcha&url=${interaction.user.displayAvatarURL({dynamic: true, extension:'png'})}&username=${interaction.user.username}`));
            let json = await res.json();
            let attachment = new discord.AttachmentBuilder(json.message,  { name: "captcha.png" });
            await interaction.editReply({files: [attachment]});
        } else {
