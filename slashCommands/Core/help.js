...
module.exports = {
  ...
  async execute(client, interaction) {
    ...
    const button10 = new ButtonBuilder()
      ...
      .setURL(
        `https://discord.com/api/oauth2/authorize?client_id=821655420410003497&permissions=8&scope=bot%20applications.commands`
      )
      .setLabel("Add me");
    ...
    const help = new discord.EmbedBuilder()
      ...
      .setDescription(
        `<a:Right:877975111846731847> Type \`${client.config.prefix}feedback\` to report a bug`
      )
      .setFooter({
    ...
    collector.on("collect", async (interactionCreate) => {
      if (interactionCreate.customId === "1") {
        if (!interactionCreate.member.id == interaction.user.id)
          return interactionCreate.deferUpdate();
