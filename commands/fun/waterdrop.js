...
module.exports = {
    ...
    usage: '',
    group: 'Fun',
    description: 'Start playing waterdrop game',
    ...
    async execute(client, message, args) {
        ...
        const setting = new discord.EmbedBuilder()
            .setColor(`Yellow`)
            .setDescription(`<a:pp527:768869552871571496> Please wait 5 seconds as we're setting up the jumps!`)
        ...
        const waterdrop = setInterval(() => {
            ...
            const Embed = new discord.EmbedBuilder()
                ...
                .setDescription(`🧍‍♂️\n🟫\n\n\n\n\n\n🟩🟩🟩🟩🟩\n🟩${a1}${a2}${a3}🟩\n🟩${b1}${b2}${b3}🟩\n🟩${c1}${c2}${c3}🟩\n🟩🟩🟩🟩🟩`)
                .setFooter({ text: ['You have 10 seconds to jump into the water!', 'You may type "cancel" at any time to stop the game.', '(Upper left, Up, Upper Right, Left, Middle, Right, Bottom Left, Bottom, Bottom Right)'].join("\n") })
            message.channel.send({ embeds: [Embed] }).then(async message => {
                try {
                    ...
                    if (msg.first().content.toLowerCase().trim() === 'cancel') {
                        ...
                        if (msg.first().content.toLowerCase().trim() === 'upper left') {
                            ...
                            if (a2 === '🟦') {
                                if (i != 4) {
                                    message.channel.send({ content: 'GG! Please wait for the other players to jump!' })
                                } else {
                                    message.channel.send({ content: 'GG! You have won Waterdrop!!' })
                                    clearInterval(waterdrop)
                                }
                            } else {
                                title = 'You lose!'
                            ...
                            message.channel.send({ content: 'Incorrect input, you completely missed the water and died!' })
                            b2 = '🟥'
                            clearInterval(waterdrop)
                        }
                    }
                ...
                if (a1 == '🟥' || a2 == '🟥' || a3 == '🟥' || b1 == '🟥' || b2 == '🟥' || b3 == '🟥' || c1 == '🟥' || c2 == '🟥' || c3 == '🟥' || d == '🟥') {
                    const embedLose = new discord.EmbedBuilder()
                        ...
                        .setColor(15158332)
                    message.channel.send({ embeds: [embedLose] })
                }
            })
        }, 5100)
    }
