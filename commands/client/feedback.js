...
module.exports = {
  ...
  permissions: [],
  clientPermissions: ["UseExternalEmojis", "AddReactions"],
  examples: [
  ...
  async execute(client, message, args) {
    ...
    if (!args.length) {
      ...
      return message.channel.send(`<a:Wrong:812104211361693696> | ${message.author}, Please add an issue to your message!`);
    };

    if (args.join(' ').length > 1000) {
      client.commands.cooldowns.get(this.name).users.delete(message.author.id);
      return message.channel.send(`<a:Wrong:812104211361693696> | ${message.author}, Please make your report brief and short! (MAX 1000 characters!)`);
    };

    const owner = await client.users.fetch(client.owners[0]).catch(() => null);

    if (!owner) {
      return message.channel.send(`<a:Wrong:812104211361693696> | ${message.author}, Couldn't contact \`owner\`!`);
    };
    ...
    if (TimeOutData.feedback > now) {
      ...
      owner.send({ embeds: [embed] }).then(() => message.react('758141943833690202')).catch(() => message.reply('<:Verify:841711383191879690> Feedback Sent!'))
        .catch(() => message.channel.send(`💢 **${owner.username}** is currently not accepting any Feedbacks right now via DMs.`));
    }
  }
};
