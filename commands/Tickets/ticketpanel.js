...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        async function Final(message, embed) {
            ...
            if(msgI == '1') {
                ...
                return await message.channel.send(embed).then(() => {
                    message.channel.send(`\\✔️ **${message.author}**, Successfully send the \`message\` to ${Embedchannel}!`)
                })
            } else {
        ...
        await message.reply(`Do you want to set a tickets category for this panel? \`(y/n)\``);
        ...
        if (!proceed){
          return await message.channel.send(Embed(message, "ticket"))
        };

        await message.channel.send(`${message.author}, Please type the tickets category id for this panel!`)

        const channel = await message.guild.channels.cache.get(MessageInput(message));
