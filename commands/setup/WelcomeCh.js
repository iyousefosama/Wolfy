...
module.exports = {
    ...
    async execute(client, message, [channel='']) {
        ...
        .then(() => {
            ...
            .setColor('DARK_GREEN')
            .setFooter(`Member Greeter | \©️${new Date().getFullYear()} Wolf`)
            .setDescription([
              '<a:Correct:812104211386728498>\u2000\u2000|\u2000\u2000',
              `Successfully set the welcome channel to ${channel}!\n\n`,
              data.greeter.welcome.channel == channel.id `\\⚠️ Welcome greeter is already set at ${channel}`,
              !data.greeter.welcome.isEnabled ? `\\⚠️ Welcome greeter is disabled! To enable, type \`${prefix}welcometoggle\`\n` :
