...
module.exports = {
    ...
    async execute(client, message, [type = '', ...args]) {
        ...
        if(type.toLowerCase() == 'add') {
            ...
            if(!wordToBeAdded) return message.channel.send(`\\âŒ **${message.member.displayName}**, Please add the word to blacklist!`)
            if(data.Mod.BadWordsFilter.BDW.includes(wordToBeAdded)) return message.channel.send(`\\âŒ **${message.member.displayName}**, This word is already blacklisted!`)
            if(data.Mod.BadWordsFilter.BDW.length > 400) return message.channel.send(`\\âŒ **${message.member.displayName}**, Maximum number of blacklisted words is (400)!`)
            ...
            .then(() => {
                ...
                .setDescription([
                  ...
                  !data.Mod.BadWordsFilter.isEnabled ? `\\âš ï¸ BadWords filter is disabled! To enable, type \`${client.prefix}badwordstoggle\`\n` :
        ...
        if(type.toLowerCase() == 'remove') {
            ...
            if(!wordToBeRemoved) return message.channel.send(`\\âŒ **${message.member.displayName}**, Please add the word to remove!`)
            if(!data.Mod.BadWordsFilter.BDW.includes(wordToBeRemoved)) return message.channel.send(`\\âŒ **${message.member.displayName}**, This word is not from blacklisted words!`)
        ...
        if(!data || data.Mod.BadWordsFilter.BDW == null || data.Mod.BadWordsFilter.BDW.length == 0) return message.channel.send(`\\âŒ **${message.member.displayName}**, There is no blacklisted words in this server!`)
