  ...
  async execute(client, message, args) {
    ...
    if (data.progress.TimeReset < now) {
      ...
      return message.channel.send(
        `\\✔️ **${message.author.tag}**, Successfully refreshed the quests`
      );
    }
    ...
    if (args[0] && args[0].toLowerCase() == "claim") {
      if (data.progress.completed < 4) {
        const NotNow = new EmbedBuilder()
          ...
          .setFooter({
            text:
              message.author.tag + ` | \©️${new Date().getFullYear()} Wolfy`,
        ...
        return message.channel.send(
          `\\❌ **${message.author.tag}**, You already claimed your reward today!`
        );
      }
