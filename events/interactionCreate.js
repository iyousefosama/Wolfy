    ...
    if (interaction.isStringSelectMenu()) {
      ...
      async function SettingChannel() {
        await interaction.update({
          content: `<:Tag:836168214525509653> **${interaction.guild.name}** Set Channels list!`,
          components: [row1],
        });
        await interaction.followUp("Please type the id of the channel to set!");
        const filter = (msg) => msg.member.id == interaction.member.id;

        let iDContent = await interaction.channel.awaitMessages({
          filter,
          max: 1,
        });
        if (iDContent.first().content == "cancel")
          return interaction.channel.send({
            content: `<:error:888264104081522698>  **|** **${interaction.author.tag}**, Cancelled the \`command\`!`,
          });
        ...
        if (!channel || channel.type !== ChannelType.GuildText) {
          return interaction.followUp({
            content: `\\❌ **${interaction.member.displayName}**, please provide a valid channel ID.`,
          });
        } else if (
          !channel
            .permissionsFor(interaction.guild.members.me)
          ...
          return interaction.followUp({
            content: `\\❌ **${interaction.member.displayName}**, I need you to give me permission to send messages on ${channel} and try again.`,
          });
        } else if (
          !channel
            .permissionsFor(interaction.guild.members.me)
            .has(discord.PermissionsBitField.Flags.EmbedLinks)
        ) {
          return interaction.followUp({
            content: `\\❌ **${interaction.member.displayName}**, I need you to give me permission to embed links on ${channel} and try again.`,
          });
        }
      ...
      if (choice == "9851985198150") {
        ...
        return await data.save().then(async () => {
          const embed = new discord.EmbedBuilder()
            .setColor("DarkGreen")
            .setDescription(
              [
                ...
                !data.greeter.welcome.isEnabled
                  ? `\\⚠️ Welcome is disabled! To enable, type \`${client.prefix}welcometoggle\`\n`
          ...
          const embed = new discord.EmbedBuilder()
            ...
            .setDescription(
              [
                ...
                !data.greeter.leaving.isEnabled
                  ? `\\⚠️ Leavermsg is disabled! To enable, type \`${client.prefix}leavertoggle\`\n`
          ...
          const embed = new discord.EmbedBuilder()
            ...
            .setDescription(
              [
                ...
                !data.Mod.Logs.isEnabled
                  ? `\\⚠️ Logs channel is disabled! To enable, type \`${client.prefix}logstoggle\`\n`
        ...
        if (
          data.Mod.Suggestion.channel !== null &&
          Channel.id == data.Mod.Suggestion.channel
        ) {
          return interaction.followUp({
            content: `\\❌ **${interaction.member.displayName}**, Suggestions channel is already set to ${Channel}!`,
          });
        }
            ...
            .setDescription(
              [
                ...
                !data.Mod.Suggestion.isEnabled
                  ? `\\⚠️ Suggestion cmd is disabled! To enable, type \`${client.prefix}suggtoggle\`\n`
