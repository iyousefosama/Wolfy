...
module.exports = {
    ...
    async execute(client, guild) {
      if (!guild || !guild.available){
        ...
        const botIcon = client.user.displayAvatarURL({ extension:'png', dynamic: true, size: 128 });
        const guildName = guild.name;
        const guildIcon = guild.iconURL({dynamic: true, extension:'png', size: 512});
        ...
        .setTimestamp()
        .setFooter({ text: owner.displayName + `(${owner.id})` || "Unknown owner", iconURL: owner.displayAvatarURL({ dynamic: true }) })
